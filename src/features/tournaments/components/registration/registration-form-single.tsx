...
export function RegistrationFormSingle() {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const fileName = `${formData.first_name}_${formData.last_name}_${formData.registration_category}.pdf`.replace(/\s+/g, '_');
      pdf.save(fileName);
    } catch (error) {
    ...
    >
      <Container maxWidth="lg" sx={{ py: { xs: 2, sm: 4 }, px: { xs: 1.5, sm: 2 } }}>
        ...
        >
          ...
          {paymentGateComplete && !showReviewStep && (
            <Box
              sx={{
                position: 'sticky',
                bottom: 0,
        ...
        >
          ...
          <DialogContent className="print-content">
            <Box sx={{ py: 2 }} id="registration-success-content">
